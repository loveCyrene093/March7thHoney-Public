<div align="center">

# March7thHoney

*"少女如春风般的吹息，挠得你耳尖与心尖一样痒痒。"*

[简体中文](README.md) · [English](docs/README_EN.md) · [繁體中文](docs/README_CHT.md) · [Discord](https://discord.gg/castoriceps)

![Status](https://img.shields.io/badge/游戏服务端-运行中-4caf96?style=flat-square)
![Battle](https://img.shields.io/badge/战斗-已支持-5b8af0?style=flat-square)
![Gacha](https://img.shields.io/badge/抽卡-完整流程-f0a050?style=flat-square)
![Quests](https://img.shields.io/badge/任务-主线%20%2B%20支线-a06ad4?style=flat-square)

</div>

---

## 项目简介

> 一个以「**完美**」为目标的游戏服务端项目。

---

## 游戏截图

<div align="center">
<img src="image/1.png" width="30%"> <img src="image/2.png" width="30%"> <img src="image/3.png" width="30%">
<img src="image/4.png" width="30%"> <img src="image/5.png" width="30%"> <img src="image/6.png" width="30%">
<img src="image/7.png" width="30%"> <img src="image/8.png" width="30%"> <img src="image/9.png" width="30%">
<img src="image/10.png" width="30%"> <img src="image/11.png" width="30%">
</div>

---

## 功能

| 系统 | 状态 | 说明 |
|---|---|---|
| 🛒 商店 | ✅ 完成 | 浏览与购买流程正常 |
| 👥 编队系统 | ✅ 完成 | 角色上阵、换位、切队流程完整 |
| 🌀 卡池抽卡 | ✅ 完成 | 支持完整抽卡流程与结果展示 |
| ⚔️ 战斗流程 | ✅ 完成 | 可进入战斗并完成主要战斗环节 |
| 🗺️ 大世界场景 | ✅ 完成 | 场景加载、交互、基础探索可用 |
| 📈 角色养成 | ✅ 完成 | 升级、晋阶等核心养成链路可用 |
| 📜 任务系统 | ✅ 完成 | 主线与常见任务流程可推进 |
| 🤝 好友系统 | ✅ 完成 | 基础好友交互与展示功能可用 |
| ⚡ 异相仲裁 | ✅ 完成 | 支持入口解锁、关卡挑战、阶段奖励领取 |
| 🏆 挑战玩法 | ✅ 完成 | 忘却之庭 · 虚构叙事 · 末日幻影可进入并游玩 |
| 💰 货币战争 | 🔧 部分完成 | 已实现基础玩法流程 |
| 🏅 成就系统 | ✅ 完成 | 大部分成就可追踪与完成 |

详细开发进度：[EN](docs/GameplayImplementationTree.md) | [简中](docs/GameplayImplementationTree_zh-CN.md) | [繁中](docs/GameplayImplementationTree_zh-TW.md) | [JP](docs/GameplayImplementationTree_ja-JP.md)

---

## 待办列表

- [ ] 完善货币战争 & 差分宇宙
- [ ] 补齐少量任务与特殊触发逻辑
- [ ] 持续优化部分玩法的结算与同步稳定性
- [ ] 提升活动类玩法的覆盖范围
- [ ] 继续修复少量已知 UI/状态同步异常
- [ ] 增加更多可配置化选项

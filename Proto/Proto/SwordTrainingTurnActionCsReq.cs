using System;
using System.CodeDom.Compiler;
using System.Diagnostics;
using Google.Protobuf;
using Google.Protobuf.Collections;
using Google.Protobuf.Reflection;

namespace March7thHoney.Proto;

[DebuggerDisplay("{ToString(),nq}")]
public sealed class SwordTrainingTurnActionCsReq : IMessage<SwordTrainingTurnActionCsReq>, IMessage, IEquatable<SwordTrainingTurnActionCsReq>, IDeepCloneable<SwordTrainingTurnActionCsReq>, IBufferMessage
{
	private static readonly MessageParser<SwordTrainingTurnActionCsReq> _parser = new MessageParser<SwordTrainingTurnActionCsReq>(() => new SwordTrainingTurnActionCsReq());

	private UnknownFieldSet _unknownFields;

	public const int ACBIMOGNKAJFieldNumber = 10;

	private static readonly FieldCodec<uint> _repeated_aCBIMOGNKAJ_codec = FieldCodec.ForUInt32(82u);

	private readonly RepeatedField<uint> aCBIMOGNKAJ_ = new RepeatedField<uint>();

	public const int AKHJFKNPGEEFieldNumber = 14;

	private static readonly FieldCodec<uint> _repeated_aKHJFKNPGEE_codec = FieldCodec.ForUInt32(114u);

	private readonly RepeatedField<uint> aKHJFKNPGEE_ = new RepeatedField<uint>();

	[DebuggerNonUserCode]
	[GeneratedCode("protoc", null)]
	public static MessageParser<SwordTrainingTurnActionCsReq> Parser => _parser;

	[DebuggerNonUserCode]
	[GeneratedCode("protoc", null)]
	public static MessageDescriptor Descriptor => SwordTrainingTurnActionCsReqReflection.Descriptor.MessageTypes[0];

	[DebuggerNonUserCode]
	[GeneratedCode("protoc", null)]
	MessageDescriptor IMessage.Descriptor => Descriptor;

	[DebuggerNonUserCode]
	[GeneratedCode("protoc", null)]
	public RepeatedField<uint> ACBIMOGNKAJ => aCBIMOGNKAJ_;

	[DebuggerNonUserCode]
	[GeneratedCode("protoc", null)]
	public RepeatedField<uint> AKHJFKNPGEE => aKHJFKNPGEE_;

	[DebuggerNonUserCode]
	[GeneratedCode("protoc", null)]
	public SwordTrainingTurnActionCsReq()
	{
	}

	[DebuggerNonUserCode]
	[GeneratedCode("protoc", null)]
	public SwordTrainingTurnActionCsReq(SwordTrainingTurnActionCsReq other)
		: this()
	{
		aCBIMOGNKAJ_ = other.aCBIMOGNKAJ_.Clone();
		aKHJFKNPGEE_ = other.aKHJFKNPGEE_.Clone();
		_unknownFields = UnknownFieldSet.Clone(other._unknownFields);
	}

	[DebuggerNonUserCode]
	[GeneratedCode("protoc", null)]
	public SwordTrainingTurnActionCsReq Clone()
	{
		return new SwordTrainingTurnActionCsReq(this);
	}

	[DebuggerNonUserCode]
	[GeneratedCode("protoc", null)]
	public override bool Equals(object other)
	{
		return Equals(other as SwordTrainingTurnActionCsReq);
	}

	[DebuggerNonUserCode]
	[GeneratedCode("protoc", null)]
	public bool Equals(SwordTrainingTurnActionCsReq other)
	{
		if (other == null)
		{
			return false;
		}
		if (other == this)
		{
			return true;
		}
		if (!aCBIMOGNKAJ_.Equals(other.aCBIMOGNKAJ_))
		{
			return false;
		}
		if (!aKHJFKNPGEE_.Equals(other.aKHJFKNPGEE_))
		{
			return false;
		}
		return object.Equals(_unknownFields, other._unknownFields);
	}

	[DebuggerNonUserCode]
	[GeneratedCode("protoc", null)]
	public override int GetHashCode()
	{
		int num = 1;
		num ^= aCBIMOGNKAJ_.GetHashCode();
		num ^= aKHJFKNPGEE_.GetHashCode();
		if (_unknownFields != null)
		{
			num ^= _unknownFields.GetHashCode();
		}
		return num;
	}

	[DebuggerNonUserCode]
	[GeneratedCode("protoc", null)]
	public override string ToString()
	{
		return JsonFormatter.ToDiagnosticString(this);
	}

	[DebuggerNonUserCode]
	[GeneratedCode("protoc", null)]
	public void WriteTo(CodedOutputStream output)
	{
		output.WriteRawMessage(this);
	}

	[DebuggerNonUserCode]
	[GeneratedCode("protoc", null)]
	void IBufferMessage.InternalWriteTo(ref WriteContext output)
	{
		aCBIMOGNKAJ_.WriteTo(ref output, _repeated_aCBIMOGNKAJ_codec);
		aKHJFKNPGEE_.WriteTo(ref output, _repeated_aKHJFKNPGEE_codec);
		if (_unknownFields != null)
		{
			_unknownFields.WriteTo(ref output);
		}
	}

	[DebuggerNonUserCode]
	[GeneratedCode("protoc", null)]
	public int CalculateSize()
	{
		int num = 0;
		num += aCBIMOGNKAJ_.CalculateSize(_repeated_aCBIMOGNKAJ_codec);
		num += aKHJFKNPGEE_.CalculateSize(_repeated_aKHJFKNPGEE_codec);
		if (_unknownFields != null)
		{
			num += _unknownFields.CalculateSize();
		}
		return num;
	}

	[DebuggerNonUserCode]
	[GeneratedCode("protoc", null)]
	public void MergeFrom(SwordTrainingTurnActionCsReq other)
	{
		if (other != null)
		{
			aCBIMOGNKAJ_.Add(other.aCBIMOGNKAJ_);
			aKHJFKNPGEE_.Add(other.aKHJFKNPGEE_);
			_unknownFields = UnknownFieldSet.MergeFrom(_unknownFields, other._unknownFields);
		}
	}

	[DebuggerNonUserCode]
	[GeneratedCode("protoc", null)]
	public void MergeFrom(CodedInputStream input)
	{
		input.ReadRawMessage(this);
	}

	[DebuggerNonUserCode]
	[GeneratedCode("protoc", null)]
	void IBufferMessage.InternalMergeFrom(ref ParseContext input)
	{
		uint num;
		while ((num = input.ReadTag()) != 0 && (num & 7) != 4)
		{
			switch (num)
			{
			default:
				_unknownFields = UnknownFieldSet.MergeFieldFrom(_unknownFields, ref input);
				break;
			case 80u:
			case 82u:
				aCBIMOGNKAJ_.AddEntriesFrom(ref input, _repeated_aCBIMOGNKAJ_codec);
				break;
			case 112u:
			case 114u:
				aKHJFKNPGEE_.AddEntriesFrom(ref input, _repeated_aKHJFKNPGEE_codec);
				break;
			}
		}
	}
}

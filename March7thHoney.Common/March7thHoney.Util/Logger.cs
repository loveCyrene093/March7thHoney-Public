...
public class Logger
{
	...
	{
		_003CmoduleName_003EP = moduleName;
		base._002Ector();
	}
	...
	public void Log(string message, LoggerLevel level)
	{
		lock (Lock)
		{
			...
			string value3 = Markup.Escape(_003CmoduleName_003EP);
			...
			try
			{
				if (!Console.IsOutputRedirected)
				{
					...
					IConsole.WriteExternalLine(plainLine: $"[{value}] [{_003CmoduleName_003EP}] {level} {message}", colorWriter: delegate
					{
				...
				{
					Console.WriteLine($"[{value}][{_003CmoduleName_003EP}] {message}");
				}
			}
			catch
			{
			...
			string message2 = $"[{value}][{_003CmoduleName_003EP}][{level}] {message}";

...
public class GridFightInstance(PlayerInstance player, uint season, uint divisionId, bool isOverLock, uint uniqueId)
{
	...
	public uint Gold { get; set; } = 3u;

	public uint LineupHp { get; set; } = 80u;

	public uint LineupMaxHp { get; } = 100u;

	public uint BattleMaxHp { get; } = 10939u;

	public uint Level { get; } = 1u;

	public uint SectionId { get; set; } = 1u;

	public uint NDOCIKPLKIF { get; } = 1801u;

	public uint CurrentChapterId { get; set; } = 1u;

	public uint CurrentBranchId { get; set; } = 1u;

	public uint KeepWinCnt { get; set; }

	public int BattlesFinished { get; set; }

	public GridFightBattleComponent BattleComponent { get; } = new GridFightBattleComponent();

	public uint QueuePosition { get; set; } = 1u;

	public GridFightPendingAction? PendingAction { get; set; }

	public uint NextRoleUniqueId { get; set; } = 100u;

	public uint NextEquipUniqueId { get; set; } = 50u;

	public Dictionary<uint, uint> RoleByUniqueId { get; } = new Dictionary<uint, uint>();

	public Dictionary<uint, uint> UniqueIdByPos { get; } = new Dictionary<uint, uint>();

	public Dictionary<uint, uint> RoleStarByUniqueId { get; } = new Dictionary<uint, uint>();

	public uint ShopRefreshLeft { get; set; } = 2u;

	public List<GridFightShopGoodsInfo> ShopGoods { get; } = new List<GridFightShopGoodsInfo>();

	public List<uint> ShopRolePool { get; } = new List<uint>
	{
		1217u, 1314u, 1108u, 1220u, 1406u, 1001u, 1009u, 1306u, 1403u, 1502u,
		1408u, 1321u, 1409u, 1402u
	};

	public List<uint> StageCandidates { get; } = new List<uint> { 35030205u, 35030405u, 35030208u, 350202u, 35030606u };

	public List<uint> CurrentEquipDraft { get; private set; } = new List<uint> { 35030205u, 35030405u, 35030208u };

	public List<GridFightEquipmentInfo> Equipments { get; } = new List<GridFightEquipmentInfo>
	{
	...
	public List<GridFightConsumableInfo> Consumables { get; } = new List<GridFightConsumableInfo>
	{
	...
	public List<uint> SectionAffixIds { get; } = new List<uint> { 1002u, 3005u, 4010u };

	public List<uint> ActivePortalBuffIds { get; } = new List<uint> { 106u };

	public List<uint> AvailablePortalBuffRerolls { get; } = new List<uint> { 105u, 1014u, 132u };
